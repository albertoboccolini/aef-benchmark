...
def event_is_matching(event, ai_event) -> bool:
    ...
    distance_between_events = geodesic(
        (ai_event["lat"], ai_event["lon"]), (event_location[0], event_location[1])
    ).km
    ...
    if (
        abs((ai_event["start"] - datetime.fromisoformat(event["start_local"])).days)
        <= 5
    ):
        score += 1
