...
def generate_evaluation_row(model_name, matching_events, non_matching_events):
    matching_scores = [
        event["score"] for event in matching_events if event["score"] is not None
    ]

    non_matching_scores = [
        event["score"] for event in non_matching_events if event["score"] is not None
    ]
    ...
    return {
        ...
        "avg_non_matching_score": sum(non_matching_scores) / len(non_matching_scores)
        if non_matching_scores
        else 0,
    }
...
def save_evaluation_results(rows, address, start_date, end_date, distance):
    ...
    for row in rows:
        ...
        results_data["AVG Matching Score (distance > 2km)"].append(
            round(row["avg_matching_score"], 3)
        )
        results_data["AVG Non-Matching Score"].append(
            round(row["avg_non_matching_score"], 3)
        )

    results_df = pd.DataFrame(results_data)

    clean_address = "".join(
        c for c in address if c.isalnum() or c in (" ", "-", "_")
    ).rstrip()
